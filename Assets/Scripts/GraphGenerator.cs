   ...
   void CreateNode(string nodeName, bool stat , int index, int totalNodes, int columns)
{
    ...
    Debug.Log($"Node {nodeName} created at column {col}, row {row}, position: {rectTransform.anchoredPosition}");
}
  ...
  void CreateEdge(Edge edge)
{
    ...
    {
        Debug.LogError($"Edge references unknown nodes: {edge.from} -> {edge.to}");
        return;
    }
    ...
    // Debug positions for verification
    Debug.Log($"Creating edge from {edge.from} at {fromPosition} to {edge.to} at {toPosition}");
    ...
    Debug.Log($"Edge created between {edge.from} and {edge.to}");
}
